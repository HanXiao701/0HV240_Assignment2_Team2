...
*** Exploring the data ***
summarize 						
*Only keep what we are going to use in this assignment
...
*Renaming variables to names that are easier to use later on
...
tab student // lots of high numbers give problems with graphs later on, so we maybe want to recode this variable
...
*Dropping those students that have less than 10 variables. We inspect them first (based on the frequencies we saw previously) to see how their answers are distributed. Often there is only one observation on a day, so it seems useless to use this data. We drop these participants. 
...
// Give students a number that we can use to prevent overlapping in later graphs
egen studentnr = group(student)
tab studentnr
...
* specific day, let's call it [timeonday]. We use this bit that was provided by Karin * in the assignment
egen day=group(Date)
tab Date day // works as expected

sort student day 
by student: gen time=_n			 
by student day: gen timeonday=_n 
drop if Date_submitted == "" 	
...
replace sleepiness = "6" if sleepiness == "Sleepy, woozy, struggling to sleep, prefer to lie down" // different answers for different years, checked manually
...
pwcorr sleepy energy stress happy motivationstd, sig star(0.05)	//lots of correlations (but nested data, so this is logical). All correlations make sense (we recoded sleepiness and stress after doing this because they seemed to be in the wrong direction (i.e. high stress score meant someone was calm and high sleepiness score meant someone was very awake))
...
// Energy and happiness are our variables of interest --> because you can also feel tired and happy, but there is a positive correlation when we look at the group level: could be interesting to look at within/between differences
...
estat icc 				
predict residuals_unconditional_mixed, res	
...
//Random slope model
mixed happiness energylevel	 || student:energylevel // p<0.05 so a random slope does make sense to add in the model 
estimates store C
...
lrtest A B // random intercept is better since p<0.05 (0.000)
lrtest B C // random slope and random intercept is better since p<0.05 (0.0024)



**2-level or 3-level model? 
//to test this, we will use unconditional models 
...
*** Centering of the variables for variables of interest
...
//run models with grand-mean scores for energy as predictor
...
swilk residuals_D			//normality is once again rejected ... 

mixed happiness Energy_GMC || student:Energy_GMC ||day:		//random intercepts + random slope (at participant level)
predict residuals_E, res	
swilk residuals_E			// normality of residuals is rejected
...
//compute deviations from the person's mean
